...
float Processor::Utilization() { //compute and return the cpu utilization
  vector<string> cpu_usage = LinuxParser::CpuUtilization();
  const long int usertime = std::stol(cpu_usage[0]);  //stol converts string vals to long integers
  const long int nicetime = std::stol(cpu_usage[1]);  // go to each vector index and convert to integer
  ...
  if (prevuser == -1 || prevnice == -1 || prevsystem == -1 || previdle == -1 ||
    ...
    // Save current CPU status in temp varriables for future computations
    prevuser = usertime;
    prevnice = nicetime;
    ...
    // Save current CPU status in temp variables
    prevuser = usertime;
    prevnice = nicetime;
    prevsystem = systemtime;
    previdle = idletime;
